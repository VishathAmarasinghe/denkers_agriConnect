  ...
  async (payload: { email: string; password: string }, { dispatch, rejectWithValue }) => {
    return new Promise<string>(async (resolve, reject) => {
      try {
        ...
        const response = await APIService.getInstance().post(`${AppConfig.serviceUrls.authenticaion}/mobile/login`, {
          email: payload.email,
          password: payload.password,
        });
        dispatch(showSnackbar({ message: 'Login Successfull', type: 'success' }));
...
export const resetPassword = createAsyncThunk(
  ...
  async (payload: { email: string; password: string; employeeID: string }, { dispatch, rejectWithValue }) => {
    return new Promise<string>(async (resolve, reject) => {
      try {
        const response = await APIService.getInstance().post(`${AppConfig.serviceUrls.authenticaion}/reset`, payload);
